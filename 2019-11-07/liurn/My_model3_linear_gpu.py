...
checkpoint_num = 0
need_train = True
model_saved_path = "../res_mat_nuswide/model_"+str(model_num)+"_" + str(num) + "_layers" + str(layers) + "_nbits" + str(
...
class My_model(nn.Module):
    def __init__(self, fsdh_input_dim, fsdh_hidden_1, fsdh_hidden_2, fsdh_out_dim, batch_size, layers, beta=0.7,
                 gamma=1e-3,
                 alpha=1.0, mu=0.):
...
#       self.linears = nn.ModuleList([nn.Sequential(nn.Linear(fsdh_input_dim, fsdh_out_dim),nn.BatchNorm1d(fsdh_out_dim), nn.ReLU()) for i in range(layers)])
#       self.linears = nn.ModuleList([nn.Sequential(nn.Linear(fsdh_input_dim, fsdh_hidden_1), nn.BatchNorm1d(fsdh_hidden_1), nn.ReLU(),nn.Linear(fsdh_hidden_1, fsdh_out),nn.Linear(fsdh_hidden_2,fsdh_out_dim)) for i in range(layers)])
        self.linears = nn.ModuleList([nn.Linear(fsdh_input_dim, fsdh_out_dim) for i in range(layers)])
        self.layer_init = nn.Linear(fsdh_input_dim, fsdh_out_dim)  # nn.BatchNorm1d(fsdh_out_dim)
