...
class Cifar_Dataset(data.Dataset):
    ...
    def __init__(self, root, train):
        ...
        self.vec = m
        if train == 1:
            self.data = torch.from_numpy(db['train_data'])
            self.label = torch.from_numpy(db['train_label'])[0]
        elif train == 2:
            self.data = torch.from_numpy(db['test_data'])
            self.label = torch.from_numpy(db['test_label'])[0]
        else:
            self.data = torch.from_numpy(db['train_data'])
            self.label = torch.from_numpy(db['train_label'])[0]
